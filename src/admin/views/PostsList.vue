...
const { items, loading, error, load, remove } = useCollection('posts');
onMounted(load);

function edit(row: CollectionItem) {
  navigate(`#/posts/edit/${row.locale}/${row.slug}`);
}
async function onRemove(row: CollectionItem) {
  if (confirm(`Delete post ${row.locale}/${row.slug}?`)) await remove(row.path);
}
...
<template>
  <section>
    <header class="head">
      <h1>Posts</h1>
      <a class="btn" href="#/posts/new">Add new</a>
    </header>
